document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() { if (!PROMPT_TEMPLATE) PROMPT_TEMPLATE = 'Please use agent-action format.'; }, 1000);
    ...
    chrome.storage.local.get(
        ...
        function(r) {
            ...
            queryActiveTabPort(function() { checkConnection(); });
        }
    );

    function queryActiveTabPort(callback) {
        chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
            if (!tabs[0]) { if (callback) callback(); return; }
            try {
                chrome.tabs.sendMessage(tabs[0].id, { type: 'get-current-port' }, function(resp) {
                    if (chrome.runtime.lastError) { if (callback) callback(); return; }
                    if (resp && resp.port) currentPort = resp.port;
    ...
    function renderInstances(instances) {
        ...
        instances.forEach(function(inst) {
            ...
            chip.addEventListener('click', function() {
                ...
                chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
                    if (tabs[0]) chrome.tabs.sendMessage(tabs[0].id, { type: 'reconnect-ws', port: inst.port });
                });
    ...
    document.getElementById('btn-context').addEventListener('click', function() {
        var btn = document.getElementById('btn-context');
        btn.disabled = true; btn.textContent = '\u23f3 \u6536\u96c6\u4e2d...';
        fetch('http://127.0.0.1:' + currentPort + '/collect-context', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}'
        }).then(function(r) { return r.json(); }).then(function(d) {
            if (d.success && d.text) {
                chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
                    if (tabs[0]) {
                        chrome.tabs.sendMessage(tabs[0].id, { type: 'inject-text-to-input', text: d.text });
                        showResult('\u2705 \u4e0a\u4e0b\u6587\u5df2\u6ce8\u5165 (' + d.text.length + ' \u5b57\u7b26)', true);
                    }
                });
            } else { showResult('\u274c ' + (d.message || '\u6536\u96c6\u5931\u8d25'), false); }
        }).catch(function() { showResult('\u274c \u65e0\u6cd5\u8fde\u63a5 VS Code', false); })
        .finally(function() { btn.disabled = false; btn.textContent = '\ud83e\udde0 \u4e0a\u4e0b\u6587\u6536\u96c6'; });
    });

    document.getElementById('btn-send-dir').addEventListener('click', function() {
        var dirPath = prompt('\u8f93\u5165\u76ee\u5f55\u8def\u5f84\uff08\u76f8\u5bf9\u4e8e\u5de5\u4f5c\u533a\uff09:', 'src');
        if (!dirPath) return;
        var btn = document.getElementById('btn-send-dir');
        btn.disabled = true; btn.textContent = '\u23f3 \u6536\u96c6\u4e2d...';
        fetch('http://127.0.0.1:' + currentPort + '/send-directory', {
            ...
            if (d.success && d.text) {
                chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
                    if (tabs[0]) {
                        chrome.tabs.sendMessage(tabs[0].id, { type: 'inject-text-to-input', text: d.text });
                        showResult('\u2705 \u5df2\u6ce8\u5165 ' + (d.fileCount || '') + ' \u4e2a\u6587\u4ef6', true);
                    }
                });
            } else { showResult('\u274c ' + (d.message || '\u5931\u8d25'), false); }
        }).catch(function() { showResult('\u274c \u65e0\u6cd5\u8fde\u63a5', false); })
        .finally(function() { btn.disabled = false; btn.textContent = '\ud83d\udcc1 \u53d1\u9001\u76ee\u5f55'; });
    });
    ...
    manualSendBtn.addEventListener('click', function() {
        ...
        .finally(function() { manualSendBtn.disabled = false; manualSendBtn.textContent = '\ud83d\udce4 \u53d1\u9001'; });
    });
    ...
    document.getElementById('btn-open-log').addEventListener('click', function() {
        chrome.runtime.sendMessage({ type: 'open-log' }); showResult('\u5df2\u6253\u5f00', true);
    });
